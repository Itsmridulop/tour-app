...
export default function CreateBookingForm({
  ...
  const { updateUser, isPending: isUserPending } = useUpdateUser();
  const { user } = useUser();
  const { id } = useParams();

  const onSubmit: SubmitHandler<FormInputs> = (data) => {
    if (tour) {
      if (data.members > tour?.data?.maxGroupSize)
        showAlert("Members should be lesser then group size.");
      const userData = {
        ...user,
        resentBooked: tour?.data.name,
      };
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-background">
      <CardHeader>
        ...
        <CardDescription>
          Enter booking details and select a payment method.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Button type="submit" className="w-full mt-4">
            {isPending || isUserPending ? "Creating..." : "Create Booking"}
          </Button>
        </form>
      </CardContent>
    </div>
